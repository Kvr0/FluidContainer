#> fluidcontainer:fluidbucket/private/used_lava
# @within function fluidcontainer:fluidbucket/private/used_bucket

## Decrease Amount
    scoreboard players remove $Lava Temporary 1000
...
### Empty Check
    execute if score $Lava Temporary matches ..0999 run data modify storage api: Item.id set value "minecraft:bucket"
    execute if score $Lava Temporary matches ..0999 run data modify storage api: Item.tag.display.Name set value '[{"text":"FluidBucket [ ","color":"reset","italic":false},{"text":"Empty","color":"white","italic":false},{"text":" ]","color":"reset","italic":false}]'
    execute if score $Lava Temporary matches ..0999 run data modify storage api: Item.tag.FluidContainer.FluidType set value "empty"
### Item Lore
